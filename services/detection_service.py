...
class DetectionService:
    ...
    def detect(self, img, source_lang=None) -> List[TextBlock]:
        ...
        bubble_detec_result = self.bubble_detection(
            img, device=self.device, imgsz=size, conf=0.1, verbose=False
        )[0]
        txt_seg_result = self.text_segmentation(
            img, device=self.device, imgsz=size, conf=0.1, verbose=False
        )[0]
        txt_detect_result = self.text_detection(
            img, device=self.device, imgsz=det_size, conf=0.2, verbose=False
        )[0]

        combined = self.combine_results(
            bubble_detec_result, txt_seg_result, txt_detect_result, img
        )

        blk_list = [
            TextBlock(txt_bbox, bble_bbox, txt_class, inp_bboxes)
    ...
    def combine_results(
        self, bubble_detec_results, text_seg_results, text_detect_results, image
    ):
        bubble_bounding_boxes = np.array(
            bubble_detec_results.boxes.xyxy.cpu(), dtype="int"
        )
        seg_text_bounding_boxes = np.array(
            text_seg_results.boxes.xyxy.cpu(), dtype="int"
        )
        detect_text_bounding_boxes = np.array(
            text_detect_results.boxes.xyxy.cpu(), dtype="int"
        )

        text_bounding_boxes = self.merge_bounding_boxes(
            seg_text_bounding_boxes, detect_text_bounding_boxes
        )
        ...
        if text_bounding_boxes is not None or len(text_bounding_boxes) > 0:
            for txt_idx, txt_box in enumerate(text_bounding_boxes):
                ...
                if not text_matched[txt_idx]:
                    raw_results.append(
                        (txt_box, None, text_blocks_bboxes[txt_idx], "text_free")
                    )
    ...
    def merge_bounding_boxes(self, seg_boxes, detect_boxes):
        ...
        for detect_box in detect_boxes:
            ...
            for merged_box in merged_boxes:
                if self.do_rectangles_overlap(
                    detect_box, merged_box, 0.1
                ) or does_rectangle_fit(merged_box, detect_box):
                    add_box = False
                    break
            if add_box:
        ...
        for i, box in enumerate(merged_boxes):
            ...
            for j, other_box in enumerate(merged_boxes):
                if i == j:
                    continue
                if self.do_rectangles_overlap(
                    running_box, other_box, 0.1
                ) and not does_rectangle_fit(running_box, other_box):
        ...
        for box in final_boxes:
            ...
            for seen_box in seen_boxes:
                if np.array_equal(box, seen_box) or self.do_rectangles_overlap(
                    box, seen_box, 0.6
                ):
                    duplicate = True
                    break
    ...
    def detect_content_in_bbox(self, image):
        ...
        num_labels_white, labels_white, stats_white, centroids_white = (
            cv2.connectedComponentsWithStats(binary_white_text, connectivity=8)
        )
        num_labels_black, labels_black, stats_black, centroids_black = (
            cv2.connectedComponentsWithStats(binary_black_text, connectivity=8)
        )
        min_area = 10
        content_bboxes = []
    ...
    def filter_bounding_boxes(self, bboxes, width_tolerance=5, height_tolerance=5):
        ...
        return [
            ...
            for bbox in bboxes
            if not (
                is_close(bbox[0], bbox[2], width_tolerance)
                or is_close(bbox[1], bbox[3], height_tolerance)
            )
        ]

    def adjust_contrast_brightness(
        self, img: np.ndarray, contrast: float = 1.0, brightness: int = 0
    ):
